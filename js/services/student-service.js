...
export async function createStudentWithCredentials(studentData) {
  ...
  const existingDocs = await getRowsWhere(TABLES.STUDENTS, "admissionNumber", "==", studentData.admissionNumber);
  if (existingDocs.length > 0) {
    throw new Error(`A student with admission number "${studentData.admissionNumber}" already exists in the database.`);
  }

  const password = studentData.password || randomPassword();
  const email = admissionNumberToEmail(studentData.admissionNumber);
  ...
  return { id, uid, admissionNumber: studentData.admissionNumber, password };
}
...
export async function bulkImportStudents(rows, defaultBranch) {
  const results = { success: 0, failed: [] };
  for (const row of rows) {
    try {
      await createStudentWithCredentials({
        name: row.name || row.Name,
        admissionNumber: String(row.admissionNumber || row["Admission Number"]),
        class: row.class || row.Class,
        branch: row.branch || row.Branch || defaultBranch,
